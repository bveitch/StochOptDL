...
from nn_defn import get_initial_params_1L, forward_prop
...
def nn_train(
    ...
    init_params = get_initial_params_1L(dim, num_hidden, 10)
...
def main(num_epochs=HPARAMS['num_epochs'], plot=True, train_baseline = True, train_regularized=True, test_set = False):
    ...
    run_train_test( 'compare_solvers_newtest_%s_reg_%s_lr_%s' % (num_epochs,HPARAMS['reg'],HPARAMS['learning_rate']) ,all_data, all_labels, num_epochs, plot, test_set = test_set)
